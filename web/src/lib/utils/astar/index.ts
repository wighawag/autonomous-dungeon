...
export function search(graph: Graph, start: GridNode, end: GridNode, options?: AStarOptions) {
	graph.cleanDirty();
	options = options || {};
...
class BinaryHeap {
	...
	constructor(private scoreFunction: (v: any) => number) {
		this.content = [];
	}
	push(element: any) {
	...
	pop() {
		...
		var end = this.content.pop() as GridNode; // TODO test if null ?
	...
	bubbleUp(n: number) {
		...
		while (true) {
			...
			if (child2N < length) {
				...
				// TODO remove `as number`, what if undefined ?
